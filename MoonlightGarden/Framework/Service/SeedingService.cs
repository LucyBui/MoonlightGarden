        ...
        public void ImportMasterData(string type)
        {
            ...
            switch (masterDataType)
            {
                case MasterDataType.Zone:
                    masterDataService.Import<ZoneBase>(zones, type, item => item.Convert<MasterData>(type, item.Type));
                    break;
                case MasterDataType.Skill:
                    masterDataService.Import<Skill>(skills, type, item => item.Convert<MasterData>(type, item.Type));
                    break;
                case MasterDataType.Monster:
                    masterDataService.Import<Monster>(monsters, type, item => item.Convert<MasterData>(type, item.ZoneId));
                    break;
                case MasterDataType.Card:
                    masterDataService.Import<CardBase>(cards, type, item => item.Convert<MasterData>(type, item.Type));
                    break;
                case MasterDataType.Equip:
                    masterDataService.Import<EquipBase>(equips, type, item => item.Convert<MasterData>(type, item.EquipType));
                    break;
            }
        }
